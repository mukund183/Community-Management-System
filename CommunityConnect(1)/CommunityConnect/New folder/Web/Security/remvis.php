...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if the passcode exists in activevisitors table
    $sql_check_passcode = "SELECT * FROM activevisitors WHERE otp = '$passcode'";
    ...
    if (!$result_check_passcode) {
        ...
        // Passcode exists, delete the row from activevisitors table
        $sql_delete_passcode = "DELETE FROM activevisitors WHERE otp = '$passcode'";
        if (mysqli_query($conn, $sql_delete_passcode)) {
            echo "Visitor removed successfully!";
        } else {
